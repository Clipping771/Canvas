...
import 'package:vinci_board/core/canvas/weight_controller.dart';
...
class SpatialLayoutEngine {
  ...
  Offset computeOptimalPlacement({
    ...
    // Generate candidates
    List<Offset> candidates = _generateCandidates(
      nodeSize,
      parentBounds,
      viewportBounds,
    );
    ...
    for (var candidate in candidates) {
      ...
      if (candidateRect.left < viewportBounds.left) {
        viewportCost += viewportBounds.left - candidateRect.left;
      }
      if (candidateRect.right > viewportBounds.right) {
        viewportCost += candidateRect.right - viewportBounds.right;
      }
  ...
  List<Offset> _generateCandidates(
    Size size,
    Rect? parentBounds,
    Rect viewportBounds,
  ) {
    if (parentBounds == null) {
      ...
      return [
        ...
        Offset(
          ...
          viewportBounds.bottom - size.height - 50,
        ),
      ];
    }
    ...
    for (double pad in [80.0, 120.0, 180.0, 250.0, 500.0, 1000.0]) {
      candidates.add(
        Offset(parentBounds.left, parentBounds.bottom + pad),
      ); // Below
      // Fallbacks just in case vertical is completely blocked
      candidates.add(
        Offset(parentBounds.right + pad, parentBounds.top),
      ); // Right
    }
    return candidates;
  }
