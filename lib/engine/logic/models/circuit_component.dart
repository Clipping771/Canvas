...
abstract class CircuitComponent {
  ...
  String toSpice(Map<String, int> nodeMap) {
    ...
    String p1 = pins.length > 0 ? (nodeMap[pins[0].nodeId]?.toString() ?? '0') : '0';
    String p2 = pins.length > 1 ? (nodeMap[pins[1].nodeId]?.toString() ?? '0') : '0';
    ...
    String val = '1';
    if (metadata.containsKey('resistance')) val = '${metadata['resistance']}';
    else if (metadata.containsKey('capacitance')) val = '${metadata['capacitance']}';
    else if (metadata.containsKey('inductance')) val = '${metadata['inductance']}';
    else if (metadata.containsKey('voltage')) val = '${metadata['voltage']}';
