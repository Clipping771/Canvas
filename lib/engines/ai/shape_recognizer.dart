...
enum ShapeType { unknown, circle, star, spiral, hexagon }
...
class ShapeRecognizer {
  static ShapeType recognize(List<Offset> points) {
    ...
    if (_isHexagon(resampled)) return ShapeType.hexagon;
  ...
  static bool _isHexagon(List<Offset> points) {
    final distanceStartEnd = (points.first - points.last).distance;
    double minX = double.infinity, minY = double.infinity;
    double maxX = double.negativeInfinity, maxY = double.negativeInfinity;
    for (var p in points) {
    ...
    final maxDim = max(maxX - minX, maxY - minY);
    if (distanceStartEnd > maxDim * 0.3) return false;
    ...
    for (int i = 2; i < points.length - 2; i++) {
      ...
      final angle = (atan2(v1.dy, v1.dx) - atan2(v2.dy, v2.dx)).abs();
      final normalizedAngle = angle > pi ? 2 * pi - angle : angle;
      if (normalizedAngle > 1.0 && normalizedAngle < 2.2) {
        corners++;
        i += 4;
      }
    }
    return corners >= 5 && corners <= 7;
  }
