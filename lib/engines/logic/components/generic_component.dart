...
class ComponentDefinition {
  ...
  mnaModel; // Contains 'type' (e.g. 'resistor', 'voltage_source') and 'valueField'
...
class GenericComponent extends CircuitComponent {
  ...
  GenericComponent(Stroke stroke, this.definition)
    ...
    _pins = definition.pinDefinitions.map((pDef) {
      ...
      if (dxStr == 'left') {
        ...
        dx = double.tryParse(dxStr.toString()) ?? 0.0;
      ...
      return CircuitPin(
        ...
        direction: isInput
            ? PortDirection.input
            : (isOutput ? PortDirection.output : PortDirection.bidirectional),
  ...
  void updatePropertiesFromText(String text) {
    ...
    if (definition.mnaModel != null &&
        definition.mnaModel!['parseRegex'] != null) {
      final regex = RegExp(
        definition.mnaModel!['parseRegex'],
        caseSensitive: false,
      );
