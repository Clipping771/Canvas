  ...
  void evaluate(SimulationTick tick) {
    ...
    try {
      ...
      for (var line in lines) {
        ...
        if (expr == 'IN1') {
          result = boolStates['IN1'] ?? false;
        } else if (expr == '!IN1')
          result = !(boolStates['IN1'] ?? false);
        else if (expr == 'IN2')
          result = boolStates['IN2'] ?? false;
        else if (expr == '!IN2')
          result = !(boolStates['IN2'] ?? false);
        else if (expr == 'IN1 && IN2')
          result = (boolStates['IN1'] ?? false) && (boolStates['IN2'] ?? false);
        else if (expr == 'IN1 || IN2')
          result = (boolStates['IN1'] ?? false) || (boolStates['IN2'] ?? false);

        // Map result back to pin
        final outPin = _pins.firstWhere(
          (p) => p.name == target,
          orElse: () => _pins.first,
        );
