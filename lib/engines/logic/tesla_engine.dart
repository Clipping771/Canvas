...
import 'package:vinci_board/engines/logic/components/battery.dart';
import 'package:vinci_board/engines/logic/components/ground.dart';
import 'package:vinci_board/engines/logic/components/switch_component.dart';
import 'package:vinci_board/engines/logic/components/led.dart';
import 'package:vinci_board/engines/logic/components/logic_gates.dart';
import 'package:vinci_board/engines/logic/components/clock.dart';
import 'package:vinci_board/engines/logic/components/resistor.dart';
import 'package:vinci_board/engines/logic/components/capacitor.dart';
import 'package:vinci_board/engines/logic/components/inductor.dart';
import 'package:vinci_board/engines/logic/components/oscilloscope.dart';
import 'package:vinci_board/engines/logic/components/scriptable_chip.dart';
import 'package:vinci_board/engines/logic/components/sub_circuit.dart';
import 'package:vinci_board/engines/logic/components/motor.dart';
import 'package:vinci_board/engines/logic/components/portal.dart';
...
class TeslaEngine {
  ...
  void _registerComponents() {
    ...
    registry.register('and gate', (s) => AndGate(s));
    registry.register('or', (s) => OrGate(s));
    registry.register('or gate', (s) => OrGate(s));
    registry.register('not', (s) => NotGate(s));
    registry.register('not gate', (s) => NotGate(s));
  ...
  String generateSpiceNetlist() {
    ...
    sb.writeln('* Vinci Board SPICE Export');
  ...
  List<Stroke> _runSimulationPass(List<Stroke> strokes) {
    ...
    for (var stroke in strokes) {
      if (stroke.toolType == ToolType.text) {
        ...
        // Circuit components dragged from ComponentDrawer
        final existing = _activeComponents[stroke.id];
        if (existing != null && existing.originalStroke.text == stroke.text) {
          nextComponents[stroke.id] = existing;
        } else {
          final component = ComponentRegistry().createComponent(stroke);
          if (component != null) {
        ...
        if (existing != null &&
            existing.originalStroke.customMetadata?['destinationId'] ==
                stroke.customMetadata?['destinationId']) {
        ...
        if (portalComp.originalStroke.customMetadata?['destinationId'] ==
            null) {
    ...
    for (var stroke in strokes) {
      if (stroke.toolType == ToolType.wire) {
        ...
        if (sourceId != null &&
            targetId != null &&
            _activeComponents.containsKey(sourceId) &&
            _activeComponents.containsKey(targetId)) {
          final sourceComp = _activeComponents[sourceId]!;
          final targetComp = _activeComponents[targetId]!;

          final sourcePin = sourcePinId != null
              ? sourceComp.pins.cast<CircuitPin?>().firstWhere(
                  (p) => p?.id == sourcePinId,
                  orElse: () => null,
                )
              : sourceComp.pins.cast<CircuitPin?>().firstWhere(
          ...
          final targetPin = targetPinId != null
              ? targetComp.pins.cast<CircuitPin?>().firstWhere(
                  (p) => p?.id == targetPinId,
                  orElse: () => null,
                )
              : targetComp.pins.cast<CircuitPin?>().firstWhere(
    ...
    for (var stroke in strokes) {
      if (_activeComponents.containsKey(stroke.id)) {
        ...
        if (sourceId != null &&
          ...
          final sourceStroke = strokes.cast<Stroke?>().firstWhere(
            (s) => s?.id == sourceId,
            orElse: () => null,
          );
          final targetStroke = strokes.cast<Stroke?>().firstWhere(
            (s) => s?.id == targetId,
            orElse: () => null,
          );
          ...
          if (sourcePin != null && targetPin != null) {
            final startPoint =
                sourceStroke.bounds.center + sourcePin.relativePosition;
            final endPoint =
                targetStroke.bounds.center + targetPin.relativePosition;

            final bezierPoints = _generateBezierPoints(startPoint, endPoint);

            final isPowered =
                sourcePin.state.logic == LogicState.high ||
                sourcePin.state.voltage > 0;
            final targetColor = isPowered ? Colors.orange : Colors.grey;

            if (stroke.color != targetColor ||
                stroke.points.length != bezierPoints.length ||
                (stroke.points.isNotEmpty &&
                    stroke.points.first != bezierPoints.first) ||
                stroke.size != 4.0) {
              updatedStrokes.add(
                stroke.copyWith(
                  color: targetColor,
