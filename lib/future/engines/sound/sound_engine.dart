...
class SoundEngine {
  static final bool _isSupported = kIsWeb || !Platform.isWindows;
  ...
  SoundEngine._init() {
    if (_isSupported) {
      ...
      _readyFuture = _setupAudio();
    } else {
      _readyFuture = Future.value();
    }
  }

  Future<void> waitForReady() => _readyFuture;
  ...
  void startDrawing() {
    if (!_isReady || !_isSupported) return;
    _player?.resume();
