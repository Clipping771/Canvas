...
class _MolPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < molecule.bonds.length; i++) {
      ...
      if (bond.atomIdx1 >= molecule.atoms.length ||
          bond.atomIdx2 >= molecule.atoms.length) {
        continue;
      }
    ...
    // ── 4. Draw atom labels ───────────────────────────────
    final double atomOpacity = ((animationProgress - 0.8) / 0.2).clamp(
      0.0,
      1.0,
    );
    ...
    if (atomOpacity > 0.0) {
      ...
      for (final atom in molecule.atoms) {
        ...
        final color = _colorFor(atom.symbol).withValues(alpha: atomOpacity);

        // White background circle to erase bond line under label
        canvas.drawCircle(
          pos,
          8.5,
          Paint()..color = Colors.white.withValues(alpha: atomOpacity),
        );
    ...
    if (atomOpacity > 0.0) {
      ...
      final footerTp = TextPainter(
        text: TextSpan(
          text: label,
          style: TextStyle(
            color: strokeColor.withValues(alpha: 0.6 * atomOpacity),
            fontSize: 10,
  ...
  void _drawDoubleBond(Canvas canvas, Offset p1, Offset p2, Paint paint) {
    ...
    canvas.drawLine(
      Offset(p1.dx + nx, p1.dy + ny),
      Offset(p2.dx + nx, p2.dy + ny),
      paint,
    );
    canvas.drawLine(
      Offset(p1.dx - nx, p1.dy - ny),
