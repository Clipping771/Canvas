...
class MemoryService {
  ...
  static Future<void> addRule(String newRule) async {
    ...
    final stopWords = {'always', 'never', 'use', 'for', 'the', 'a', 'an', 'in', 'on', 'at', 'to', 'is', 'are', 'do', 'not', 'when', 'draw', 'drawing'};
    Set<String> extractKeywords(String text) {
      return text.toLowerCase().replaceAll(RegExp(r'[^\w\s]'), '').split(' ')
          .where((w) => w.length > 2 && !stopWords.contains(w)).toSet();
    }
