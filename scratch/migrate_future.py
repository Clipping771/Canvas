...
for src, dst in moves_list:
    ...
    # 'lib/adapters/ai/ai_copilot_service.dart' -> 'package:vinci_board/adapters/ai/ai_copilot_service.dart'
    ...
    try:
        ...
        for match in import_pattern.finditer(content):
            ...
            if import_str.startswith('.'):
                ...
                target_path = os.path.normpath(os.path.join(file_dir, import_str))
                target_path = target_path.replace('\\', '/')
                if target_path.startswith('lib/'):
                    resolved_pkg = target_path.replace('lib/', 'package:vinci_board/', 1)
            
            # Now we have resolved_pkg. Is it in the move map?
            final_pkg = move_map.get(resolved_pkg, resolved_pkg)
            
            # If we changed anything (relative -> package, or old package -> new package)
...
# Now perform the physical moves
for src, dst in moves_list:
