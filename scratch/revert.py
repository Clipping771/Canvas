...
# The same moves list but reversed
moves = [
...
for orig, fut in moves:
    orig_path = os.path.normpath(orig)
    fut_path = os.path.normpath(fut)
    if os.path.exists(fut_path):
        os.makedirs(os.path.dirname(orig_path), exist_ok=True)
        shutil.move(fut_path, orig_path)

# Then we write a smart fixer that will parse all dart files in the whole project
